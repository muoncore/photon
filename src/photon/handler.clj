...
(defn f-ws-projections-handler [ms]
  (fn [{:keys [ws-channel] :as req}]
    (go
      (loop [t 0]
        (if-let [{:keys [message]} (<! ws-channel)]
...
(defn f-ws-streams-handler [ms]
  (fn [{:keys [ws-channel] :as req}]
    (let [ch (streams/stream (:stm ms)
                             {"from" "0"
                              "stream-name" "__streams__"
                              "stream-type" "hot-cold"})
          current-value (atom (streams/streams (:stm ms)))]
      (go-loop [t 0]
        (if-let [{:keys [message]} (<! ws-channel)]
          (do
            (<! (timeout t))
  ...
  (defroutes* d-app-routes
    ...
    (GET* "/stream-contents/:stream-name" [stream-name]
          ...
          :summary "Obtain a list (maximum of 50) of events contained
                  in a given stream"
          (ok (api/stream (:stm ms) stream-name
                          :limit 50)))
    (GET* "/event/:stream-name/:order-id" [stream-name order-id]
          ...
          :summary "Obtain the event identified by a given stream name
                  and an order ID"
          (let [res (api/event (:stm ms) stream-name
    ...
    (POST* "/projection" request
           ...
           (ok (api/post-projection! (:stm ms) (:body request))))
    (POST* "/event" request
           :body [body api/EventTemplate]
           :return api/PostResponse
           :summary "Add an event"
           (let [res (try
                       (api/post-event! (:stm ms) (:body request))
                       (catch Exception e
...
(defn ws-routes [ms]
  (cc/defroutes m-ws-routes
    (let [ws-handler (f-ws-handler ms)
          ws-streams-handler (f-ws-streams-handler ms)
          ws-projections-handler (f-ws-projections-handler ms)]
